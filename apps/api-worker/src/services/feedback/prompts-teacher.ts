...
const INITIAL_PROMPT_INTRO = `You are a professional writing tutor specializing in academic argumentative writing. Give clear, direct feedback to help the student improve. Be constructive and specific. Keep it brief (2-3 sentences max). Don't mention technical terms like "CEFR" or "band scores" - focus on actionable improvements.`;

const INITIAL_PROMPT_GUIDE = `Focus your feedback on:
...
const CLUES_PROMPT_INTRO = `You are a professional writing tutor. The student tried again but still has issues. Give them specific clues (not full answers) to guide them. Be constructive. Keep it brief (2-3 sentences).`;

const CLUES_PROMPT_GUIDE = `Give clues that help them identify the problems themselves. Focus on essay structure, addressing all parts of the question, connecting ideas, or vocabulary/grammar issues. Provide guidance without giving away all the answers.`;

const EXPLANATION_PROMPT_INTRO = `You are an experienced writing instructor analyzing a student's essay for another teacher. Provide a comprehensive, structured analysis using markdown formatting. This analysis should help the teacher understand the student's performance across all dimensions.`;

const EXPLANATION_PROMPT_GUIDE = `Write a detailed teacher-to-teacher analysis in markdown format with the following structure:
...
export function buildTeacherFeedbackPrompt(
  ...
  const contextBlock = [scoreContext, wordCountContext, errorContext, relevanceContext]
    .filter(Boolean)
    .join("\n\n");

  if (mode === "initial") {
    return buildTeacherPrompt({
      intro: INITIAL_PROMPT_INTRO,
      questionText,
      answerText: truncatedAnswerText,
      contextBlock,
      guide: INITIAL_PROMPT_GUIDE,
    });
  }

  if (mode === "clues") {
    return buildTeacherPrompt({
      intro: CLUES_PROMPT_INTRO,
      questionText,
      answerText: truncatedAnswerText,
      contextBlock,
      guide: CLUES_PROMPT_GUIDE,
    });
  }

  return buildTeacherPrompt({
    intro: EXPLANATION_PROMPT_INTRO,
    questionText,
    answerText: truncatedAnswerText,
    contextBlock,
    guide: EXPLANATION_PROMPT_GUIDE,
  });
}
