...
test.describe("Draft Tracking", () => {
  ...
  test("TC-DRAFT-028: Page reload preserves results in local mode", async ({
    writePage,
    resultsPage,
    page,
  }) => {
    ...
    const submissionId = resultsUrl.match(/\/results\/([a-f0-9-]+)/)?.[1];
    expect(submissionId).toBeTruthy();

    // Verify results are displayed
    const overallScore = await resultsPage.getOverallScore();
    await expect(overallScore.first()).toBeVisible({ timeout: 5000 });

    // Reload the page
    await page.reload();
    await resultsPage.waitForResults();

    // Verify results are still displayed after reload (this tests our hydration fix)
    const overallScoreAfterReload = await resultsPage.getOverallScore();
    await expect(overallScoreAfterReload.first()).toBeVisible({ timeout: 5000 });
