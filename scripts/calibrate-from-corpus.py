...
def main():
    ...
    if not MODAL_URL:
        ...
        print(
            "Set it via: export MODAL_URL=https://your-username--writeo-essay-fastapi-app.modal.run"
        )
        sys.exit(1)
    ...
    for idx, row in df.iterrows():
        ...
        if model_score is not None:
            ...
            print(
                f"  Error scoring essay {idx + 1}: {result.get('error', 'Unknown error')}"
            )
    ...
    for cefr in sorted(stats.keys()):
        ...
        print(
            f"  Model Score: {stat['mean']:.2f} ± {stat['std']:.2f} (range: {stat['min']:.2f}-{stat['max']:.2f})"
        )
        print(
            f"  Target Score: {stat['target_score_min']:.1f}-{stat['target_score_max']:.1f} (mean: {stat['target_score_mean']:.2f})"
        )
