...
def load_calibration_results(input_file: str) -> dict:
    ...
    if not os.path.exists(input_file):
        ...
        print(f"Run calibrate-from-corpus.py first to generate calibration data")
        sys.exit(1)
...
def main():
    ...
    for cefr in sorted(stats.keys()):
        stat = stats[cefr]
        print(f"  {cefr}: {stat['count']} essays, adjustment: {stat['adjustment']:+.2f}")
    ...
    print(f"It's based on actual model performance on {total_essays} essays with human CEFR labels.")
